...
current_sdid_estimator_class = None
current_sdid_config_class = None
try:
    sys.path.insert(0, PROJECT_ROOT)
    from mlsynth.estimators.sdid import SDID
    from mlsynth.config_models import SDIDConfig
    current_sdid_estimator_class = SDID
    current_sdid_config_class = SDIDConfig
finally:
    sys.path = list(original_sys_path)

if not current_sdid_estimator_class or not current_sdid_config_class:
    print("Failed to import current SDID modules. Exiting.")
    sys.exit(1)

# Import LEGACY project modules
legacy_sdid_estimator_class = None
cached_modules = {}
...
try:
    ...
    from mlsynth.mlsynth import SDID as LegacySDID
    legacy_sdid_estimator_class = LegacySDID
except ImportError as e:
    print(f"Error importing legacy SDID: {e}")
    sys.exit(1)
finally:
    sys.path = list(original_sys_path)
...
if not legacy_sdid_estimator_class:
    print("Failed to import legacy SDID class. Exiting.")
    sys.exit(1)
...
def compare_sdid_outputs(legacy_res_dict, current_res_obj):
    summary = {"SDID": {}}
    method_name = "SDID"
    ...
    if legacy_cf_vector is not None and current_cf_vector is not None:
        ...
        if legacy_cf_flat.shape == current_cf_flat.shape:
            ...
            print(f"Warning: Counterfactual shapes differ for SDID. Legacy: {legacy_cf_flat.shape}, Current: {current_cf_flat.shape}")
    elif legacy_cf_vector is None and current_cf_vector is None:
        summary[method_name]["Counterfactual_match"] = True
    else:
        summary[method_name]["Counterfactual_match"] = False
        print(f"Warning: One of the counterfactuals is None for SDID. Legacy: {type(legacy_cf_vector)}, Current: {type(current_cf_vector)}")
    ...
    if legacy_weights_dict is not None and current_weights_dict is not None:
        ...
        if len(legacy_w_sorted) == len(current_w_sorted):
            ...
            print(f"Warning: Number of donors in weights differ for SDID.")
    elif legacy_weights_dict is None and current_weights_dict is None:
        summary[method_name]["Weights_match"] = True 
    else:
        summary[method_name]["Weights_match"] = False
        print(f"Warning: One of the weights dictionaries is None for SDID. Legacy: {type(legacy_weights_dict)}, Current: {type(current_weights_dict)}")
...
legacy_sdid_estimator = legacy_sdid_estimator_class(config=legacy_config_dict)
legacy_sdid_results = None
try:
    legacy_sdid_results = legacy_sdid_estimator.fit() 
    save_results(legacy_sdid_results, "legacy_sdid_outputs.json")
except Exception as e:  # Catching generic Exception for now
    print(f"!!! Legacy SDID failed to run due to an error: {e}")
    legacy_sdid_results = {"error": f"Legacy SDID failed: {e}"}
    save_results(legacy_sdid_results, "legacy_sdid_outputs.json")

# --- Run Current SDID ---
print("\n--- Running Current SDID ---")
current_sdid_config = current_sdid_config_class(
    df=df.copy(),
    unitid=UNIT_ID_COLUMN_NAME,
    time=TIME_ID_COLUMN_NAME,
    outcome=OUTCOME_VARIABLE,
...
current_sdid_estimator = current_sdid_estimator_class(config=current_sdid_config)
current_sdid_results = current_sdid_estimator.fit() 
save_results(current_sdid_results, "current_sdid_outputs.json")

# --- Compare Results ---
if legacy_sdid_results and "error" not in legacy_sdid_results:
    comparison_summary = compare_sdid_outputs(legacy_sdid_results, current_sdid_results)
    print_comparison_summary(comparison_summary)
else:
    print("\n--- Comparison Summary ---")
    print("Skipping comparison due to legacy SDID failure or no results.")
    print("Current SDID execution results saved.")
    if current_sdid_results:
        current_effects_attr = getattr(current_sdid_results, 'effects', None)
        current_att = getattr(current_effects_attr, 'att', None) if current_effects_attr else None
        print(f"  Current SDID ATT: {current_att if current_att is not None else 'N/A'}")

print("\nSDID Comparison Test Completed.")
