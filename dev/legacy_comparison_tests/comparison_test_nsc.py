...
current_nsc_estimator_class = None
current_nsc_config_class = None
try:
    sys.path.insert(0, PROJECT_ROOT)
    from mlsynth.estimators.nsc import NSC
    from mlsynth.config_models import NSCConfig
    current_nsc_estimator_class = NSC
    current_nsc_config_class = NSCConfig
finally:
    sys.path = list(original_sys_path)

if not current_nsc_estimator_class or not current_nsc_config_class:
    print("Failed to import current NSC modules. Exiting.")
    sys.exit(1)

# Import LEGACY project modules
legacy_nsc_estimator_class = None
cached_modules = {}
...
try:
    ...
    from mlsynth.mlsynth import NSC as LegacyNSC
    legacy_nsc_estimator_class = LegacyNSC
except ImportError as e:
    print(f"Error importing legacy NSC: {e}")
    sys.exit(1)
finally:
    sys.path = list(original_sys_path)
...
if not legacy_nsc_estimator_class:
    print("Failed to import legacy NSC class. Exiting.")
    sys.exit(1)
...
def compare_nsc_outputs(legacy_res_dict, current_res_obj):
    summary = {"NSC": {}}
    method_name = "NSC"
    ...
    # Current NSC JSON (and likely Pydantic model) uses 'counterfactual_outcome'
    current_cf_vector = getattr(current_time_series_attr, 'counterfactual_outcome', None) if current_time_series_attr else None
    # Removed the incorrect fallback logic that tried 'current_res_obj.vectors'
    ...
    if legacy_cf_vector is not None and current_cf_vector is not None:
        ...
        if legacy_cf_flat.shape == current_cf_flat.shape:
            ...
            print(f"Warning: Counterfactual shapes differ for NSC. Legacy: {legacy_cf_flat.shape}, Current: {current_cf_flat.shape}")
    elif legacy_cf_vector is None and current_cf_vector is None:
        summary[method_name]["Counterfactual_match"] = True
    else:
        summary[method_name]["Counterfactual_match"] = False
        print(f"Warning: One of the counterfactuals is None for NSC. Legacy: {type(legacy_cf_vector)}, Current: {type(current_cf_vector)}")

    # Weights Comparison (NSC typically has donor weights)
    # Assuming legacy NSC stores weights in a dictionary under "Weights" key, similar to other SCMs.
    # Or it might be a list of neighbor names. For now, assume dictionary of weights.
    legacy_weights_data = legacy_res_dict.get("Weights") # This could be a dict or list
    legacy_weights_dict = None
    if isinstance(legacy_weights_data, dict):
        legacy_weights_dict = legacy_weights_data
    elif isinstance(legacy_weights_data, list) and len(legacy_weights_data) > 0 and isinstance(legacy_weights_data[0], dict):
         # Handle cases like FSCM where weights are [{donor_weights_dict}, {cardinality_info}]
        legacy_weights_dict = legacy_weights_data[0]
    ...
    if legacy_weights_dict is not None and current_weights_dict is not None:
        # Ensure keys are strings for consistent sorting, especially if legacy uses numeric keys
        ...
        if len(legacy_w_sorted) == len(current_w_sorted):
            ...
            print(f"Warning: Number of donors in weights differ for NSC.")
    elif legacy_weights_dict is None and current_weights_dict is None:
            summary[method_name]["Weights_match"] = True 
    else:
        summary[method_name]["Weights_match"] = False
        print(f"Warning: One of the weights dictionaries is None for NSC. Legacy: {type(legacy_weights_dict)}, Current: {type(current_weights_dict)}")
...
K_NEIGHBORS = 5 # Common parameter for NSC
...
legacy_config_dict = {
    ...
    "k": K_NEIGHBORS, # Legacy NSC might use 'k' for k_neighbors
    "display_graphs": False,
}
legacy_nsc_estimator = legacy_nsc_estimator_class(config=legacy_config_dict)
legacy_nsc_results = None
try:
    legacy_nsc_results = legacy_nsc_estimator.fit() 
    save_results(legacy_nsc_results, "legacy_nsc_outputs.json")
except Exception as e: # Catching generic Exception for now
    print(f"!!! Legacy NSC failed to run due to an error: {e}")
    legacy_nsc_results = {"error": f"Legacy NSC failed: {e}"}
    save_results(legacy_nsc_results, "legacy_nsc_outputs.json")


# --- Run Current NSC ---
print("\n--- Running Current NSC ---")
current_nsc_config = current_nsc_config_class(
    df=df.copy(),
    unitid=UNIT_ID_COLUMN_NAME,
    time=TIME_ID_COLUMN_NAME,
    outcome=OUTCOME_VARIABLE,
    treat=TREATMENT_INDICATOR_COLUMN,
    # k=K_NEIGHBORS, # Removed, as 'k' is not a valid parameter for current NSCConfig.
    # NSCConfig uses a_search_space and b_search_space for hyperparameter tuning.
    # If K_NEIGHBORS was related to a k-means clustering step, current NSCConfig doesn't have an equivalent.
    display_graphs=False,
)
current_nsc_estimator = current_nsc_estimator_class(config=current_nsc_config)
current_nsc_results = current_nsc_estimator.fit() 
save_results(current_nsc_results, "current_nsc_outputs.json")

# --- Compare Results ---
if legacy_nsc_results and "error" not in legacy_nsc_results:
    comparison_summary = compare_nsc_outputs(legacy_nsc_results, current_nsc_results)
    print_comparison_summary(comparison_summary)
else:
    print("\n--- Comparison Summary ---")
    print("Skipping comparison due to legacy NSC failure or no results.")
    print("Current NSC execution results saved.")
    if current_nsc_results:
        current_effects_attr = getattr(current_nsc_results, 'effects', None)
        current_att = getattr(current_effects_attr, 'att', None) if current_effects_attr else None
        print(f"  Current NSC ATT: {current_att if current_att is not None else 'N/A'}")

print("\nNSC Comparison Test Completed.")
