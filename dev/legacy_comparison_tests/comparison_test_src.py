...
current_src_estimator_class = None
current_src_config_class = None
try:
    sys.path.insert(0, PROJECT_ROOT)
    from mlsynth.estimators.src import SRC
    from mlsynth.config_models import SRCConfig
    current_src_estimator_class = SRC
    current_src_config_class = SRCConfig
finally:
    sys.path = list(original_sys_path)

if not current_src_estimator_class or not current_src_config_class:
    print("Failed to import current SRC modules. Exiting.")
    sys.exit(1)

# Import LEGACY project modules
legacy_src_estimator_class = None
cached_modules = {}
...
try:
    ...
    from mlsynth.mlsynth import SRC as LegacySRC
    legacy_src_estimator_class = LegacySRC
except ImportError as e:
    print(f"Error importing legacy SRC: {e}")
    sys.exit(1)
finally:
    sys.path = list(original_sys_path)
...
if not legacy_src_estimator_class:
    print("Failed to import legacy SRC class. Exiting.")
    sys.exit(1)
...
def compare_src_outputs(legacy_res_dict, current_res_obj):
    summary = {"SRC": {}}
    method_name = "SRC"

    # ATT Comparison
    # Legacy SRC stores ATT under a top-level "ATT" key, then another "ATT" key.
    legacy_att_dict = legacy_res_dict.get("ATT", {})
    legacy_att = legacy_att_dict.get("ATT")
    ...
    # Legacy SRC has "Counterfactual" at top level (flat list) and under "Vectors" (list of lists).
    # We use the one under "Vectors" as it's more structured, though script flattens it.
    legacy_cf_vector = legacy_res_dict.get("Vectors", {}).get("Counterfactual") 
    if legacy_cf_vector is None: # Fallback to top-level if "Vectors" path fails
        legacy_cf_vector = legacy_res_dict.get("Counterfactual")

    current_time_series_attr = getattr(current_res_obj, 'time_series', None)
    # Current SRC JSON shows 'counterfactual_outcome' is populated
    ...
    if legacy_cf_vector is not None and current_cf_vector is not None:
        ...
        if legacy_cf_flat.shape == current_cf_flat.shape:
            ...
            print(f"Warning: Counterfactual shapes differ for SRC. Legacy: {legacy_cf_flat.shape}, Current: {current_cf_flat.shape}")
    elif legacy_cf_vector is None and current_cf_vector is None:
        summary[method_name]["Counterfactual_match"] = True
    else:
        summary[method_name]["Counterfactual_match"] = False
        print(f"Warning: One of the counterfactuals is None for SRC. Legacy: {type(legacy_cf_vector)}, Current: {type(current_cf_vector)}")
    ...
    if legacy_weights_dict is not None and current_weights_dict is not None:
        ...
        if len(legacy_w_sorted) == len(current_w_sorted):
            ...
            print(f"Warning: Number of donors in weights differ for SRC.")
    elif legacy_weights_dict is None and current_weights_dict is None:
        summary[method_name]["Weights_match"] = True 
    else:
        summary[method_name]["Weights_match"] = False
        print(f"Warning: One of the weights dictionaries is None for SRC. Legacy: {type(legacy_weights_dict)}, Current: {type(current_weights_dict)}")
...
legacy_src_estimator = legacy_src_estimator_class(config=legacy_config_dict)
legacy_src_results = None
try:
    legacy_src_results = legacy_src_estimator.fit() 
    save_results(legacy_src_results, "legacy_src_outputs.json")
except Exception as e:  # Catching generic Exception for now
    print(f"!!! Legacy SRC failed to run due to an error: {e}")
    legacy_src_results = {"error": f"Legacy SRC failed: {e}"}
    save_results(legacy_src_results, "legacy_src_outputs.json")

# --- Run Current SRC ---
print("\n--- Running Current SRC ---")
current_src_config = current_src_config_class(
    df=df.copy(),
    unitid=UNIT_ID_COLUMN_NAME,
    time=TIME_ID_COLUMN_NAME,
    outcome=OUTCOME_VARIABLE,
...
current_src_estimator = current_src_estimator_class(config=current_src_config)
current_src_results = current_src_estimator.fit() 
save_results(current_src_results, "current_src_outputs.json")

# --- Compare Results ---
if legacy_src_results and "error" not in legacy_src_results:
    comparison_summary = compare_src_outputs(legacy_src_results, current_src_results)
    print_comparison_summary(comparison_summary)
else:
    print("\n--- Comparison Summary ---")
    print("Skipping comparison due to legacy SRC failure or no results.")
    print("Current SRC execution results saved.")
    if current_src_results:
        current_effects_attr = getattr(current_src_results, 'effects', None)
        current_att = getattr(current_effects_attr, 'att', None) if current_effects_attr else None
        print(f"  Current SRC ATT: {current_att if current_att is not None else 'N/A'}")

print("\nSRC Comparison Test Completed.")
