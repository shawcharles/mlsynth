...
current_proximal_estimator_class = None
current_proximal_config_class = None
try:
    sys.path.insert(0, PROJECT_ROOT)
    from mlsynth.estimators.proximal import PROXIMAL
    from mlsynth.config_models import PROXIMALConfig
    current_proximal_estimator_class = PROXIMAL
    current_proximal_config_class = PROXIMALConfig
finally:
    sys.path = list(original_sys_path)

if not current_proximal_estimator_class or not current_proximal_config_class:
    print("Failed to import current Proximal modules. Exiting.")
    sys.exit(1)

# Import LEGACY project modules
legacy_proximal_estimator_class = None
cached_modules = {}
...
try:
    ...
    from mlsynth.mlsynth import PROXIMAL as LegacyPROXIMAL
    legacy_proximal_estimator_class = LegacyPROXIMAL
except ImportError as e:
    print(f"Error importing legacy PROXIMAL: {e}")
    sys.exit(1)
finally:
    sys.path = list(original_sys_path)
...
if not legacy_proximal_estimator_class:
    print("Failed to import legacy PROXIMAL class. Exiting.")
    sys.exit(1)
...
def compare_proximal_outputs(legacy_res_dict, current_res_obj):
    summary = {"Proximal": {}}
    method_name = "Proximal"
    ...
    if legacy_cf_vector is not None and current_cf_vector is not None:
        ...
        if legacy_cf_flat.shape == current_cf_flat.shape:
            ...
            print(f"Warning: Counterfactual shapes differ for Proximal. Legacy: {legacy_cf_flat.shape}, Current: {current_cf_flat.shape}")
    elif legacy_cf_vector is None and current_cf_vector is None:
        summary[method_name]["Counterfactual_match"] = True
    else:
        summary[method_name]["Counterfactual_match"] = False
        print(f"Warning: One of the counterfactuals is None for Proximal. Legacy: {type(legacy_cf_vector)}, Current: {type(current_cf_vector)}")

    # Weights Comparison
    legacy_weights_data = legacy_res_dict.get("Weights")
    ...
    if legacy_weights_dict is not None and current_weights_dict is not None:
        ...
        if len(legacy_w_sorted) == len(current_w_sorted):
            ...
            print(f"Warning: Number of donors in weights differ for Proximal.")
    elif legacy_weights_dict is None and current_weights_dict is None:
        summary[method_name]["Weights_match"] = True 
    else:
        summary[method_name]["Weights_match"] = False
        print(f"Warning: One of the weights dictionaries is None for Proximal. Legacy: {type(legacy_weights_dict)}, Current: {type(current_weights_dict)}")
...
# --- Configuration for Proximal Test ---
...
# --- Run Legacy Proximal ---
print("--- Running Legacy Proximal ---")
# Define a list of donor units (states other than California)
DONOR_UNITS = ["New York", "Texas", "Florida", "Illinois"]
PROXY_VARS_CONFIG = {"donorproxies": ["Proposition 99"], "surrogatevars": ["Proposition 99"]}
...
legacy_config_dict = {
    ...
    "donors": DONOR_UNITS,
    "vars": PROXY_VARS_CONFIG,  # Added vars for legacy
    "display_graphs": False,
}
legacy_proximal_estimator = legacy_proximal_estimator_class(config=legacy_config_dict)
legacy_proximal_results = None
try:
    legacy_proximal_results = legacy_proximal_estimator.fit() 
    save_results(legacy_proximal_results, "legacy_proximal_outputs.json")
except Exception as e:  # Catching generic Exception for now
    print(f"!!! Legacy Proximal failed to run due to an error: {e}")
    legacy_proximal_results = {"error": f"Legacy Proximal failed: {e}"}
    save_results(legacy_proximal_results, "legacy_proximal_outputs.json")

# --- Run Current Proximal ---
print("\n--- Running Current Proximal ---")
current_proximal_config = current_proximal_config_class(
    df=df.copy(),
    unitid=UNIT_ID_COLUMN_NAME,
    time=TIME_ID_COLUMN_NAME,
    outcome=OUTCOME_VARIABLE,
    treat=TREATMENT_INDICATOR_COLUMN,
    donors=DONOR_UNITS,
    vars=PROXY_VARS_CONFIG,  # Added vars for current
    display_graphs=False,
)
current_proximal_estimator = current_proximal_estimator_class(config=current_proximal_config)
current_proximal_results = current_proximal_estimator.fit() 
save_results(current_proximal_results, "current_proximal_outputs.json")

# --- Compare Results ---
if legacy_proximal_results and "error" not in legacy_proximal_results:
    comparison_summary = compare_proximal_outputs(legacy_proximal_results, current_proximal_results)
    print_comparison_summary(comparison_summary)
else:
    print("\n--- Comparison Summary ---")
    print("Skipping comparison due to legacy Proximal failure or no results.")
    print("Current Proximal execution results saved.")
    if current_proximal_results:
        current_effects_attr = getattr(current_proximal_results, 'effects', None)
        current_att = getattr(current_effects_attr, 'att', None) if current_effects_attr else None
        print(f"  Current Proximal ATT: {current_att if current_att is not None else 'N/A'}")

print("\nProximal Comparison Test Completed.")
