...
class PROXIMAL:
    ...
    def fit(self) -> List[BaseEstimatorResults]:
        ...
        if self.display_graphs:
            try:
                plot_estimates(
                    df=prepared_data, # Pass the prepared data dictionary
                    time=self.time,
                    unitid=self.unitid,
                    outcome=self.outcome,
                    treatmentname=self.treat,
                    treated_unit_name=prepared_data["treated_unit_name"], # Name of the treated unit
                    y=prepared_data["y"], # Observed outcomes of the treated unit
                    cf_list=plot_counterfactuals, # List of counterfactual series (PI, PIS, PIPost)
                    method="PI", # Base method name for plot title (can be generic for multiple series)
                    counterfactual_names=plot_counterfactual_names, # Names for legend
                    treatedcolor=self.treated_color,
                    save_path=self.save if isinstance(self.save, str) else None, # Path if save is a string
                    counterfactualcolors=self.counterfactual_color, # Colors for counterfactual lines
                )
