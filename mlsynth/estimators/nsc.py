...
class NSC:
    ...
    def fit(self) -> BaseEstimatorResults:
        ...
        if self.display_graphs:
            try:
                plot_estimates(
                    df=prepared_data, # type: ignore # prepared_data is defined in the try block
                    time=self.time,
                    unitid=self.unitid,
                    outcome=self.outcome,
                    treatmentname=self.treat,
                    treated_unit_name=prepared_data["treated_unit_name"], # type: ignore
                    y=prepared_data["y"], # type: ignore # Observed outcomes for plotting
                    cf_list=[counterfactual_outcome_series], # type: ignore # List of counterfactual series
                    counterfactual_names=[f"NSC {prepared_data['treated_unit_name']}"], # type: ignore # Names for legend
                    method="NSC", # Method name for plot title/saving
                    treatedcolor=self.treated_color,
                    counterfactualcolors=(
                        # Ensure counterfactualcolors is a list, even if a single color string is provided
