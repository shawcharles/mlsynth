    ...
    def fit(self) -> BaseEstimatorResults:
        ...
        if self.display_graphs and prepared_data and full_counterfactual_outcome_fma is not None and treated_outcome_all_periods is not None:
            try:
                ...
                plot_estimates(
                    df=prepared_data, # Data prepared by dataprep
                    ...
                    y=treated_outcome_all_periods, # Observed outcome series
                    cf_list=[full_counterfactual_outcome_fma], # List of counterfactual series (only one for FMA)
                    method="FMA", # Method name for plot title
                    treatedcolor=self.treated_color, # Color for treated line
                    counterfactualcolors=( # Color(s) for counterfactual line(s)
                    ...
                    save=self.save, # Save option from config
                )
