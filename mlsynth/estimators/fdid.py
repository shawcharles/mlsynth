        ...
        if self.display_graphs and prepared_data: 
            try:
                ...
                if list_of_counterfactual_vectors_for_plot:
                    plot_estimates(
                        df=prepared_data, # Pass prepared_data which contains necessary context like treated_unit_name.
                        time=self.time,
                        unitid=self.unitid,
                        outcome=self.outcome,
                        treatmentname=self.treated, 
                        treated_unit_name=prepared_data["treated_unit_name"],
                        y=prepared_data["y"], # Observed outcome for the treated unit.
                        cf_list=list_of_counterfactual_vectors_for_plot, # List of CF vectors (FDID, DID).
                        method="FDID", # Main method name for plot title/saving.
                        ...
                        counterfactualcolors=( # Colors for CF lines.
