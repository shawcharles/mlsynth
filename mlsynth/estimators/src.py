        ...
        if self.display_graphs: # If plotting is enabled in the configuration.
            try:
                ...
                if cf_to_plot is None or not isinstance(cf_to_plot, np.ndarray):
                    ...
                    plot_estimates(
                        df=prepared_panel_data, # Contextual data from dataprep.
                        time=self.time, # Name of the time column.
                        unitid=self.unitid, # Name of the unit ID column.
                        outcome=self.outcome, # Name of the outcome column.
                        treatmentname=self.treat, # Name of the treatment indicator column (used for plot title/label).
                        treated_unit_name=str(prepared_panel_data["treated_unit_name"]), # Name of the treated unit.
                        y=prepared_panel_data["y"], # Observed outcome series.
                        cf_list=[cf_to_plot], # List containing the counterfactual series.
                        counterfactual_names=[f"SRC {prepared_panel_data['treated_unit_name']}"], # Label for the counterfactual line.
                        method="SRC", # Method name for plot title.
                        treatedcolor=self.treated_color, # Color for the observed outcome line.
                        counterfactualcolors=( # Color(s) for the counterfactual line(s).
                            [self.counterfactual_color]
                            if isinstance(self.counterfactual_color, str)
                            else self.counterfactual_color
                        ),
                        save_path=self.save if isinstance(self.save, str) else None, # Path to save the plot, if specified.
                    )
