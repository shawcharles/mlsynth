        ...
        if self.display_graphs:
            try:
                ...
                if plot_counterfactual_series is not None:
                    ...
                    plot_estimates(
                        df=prepared_data, # type: ignore # Pass the prepared data dictionary
                        time=self.time, 
                        unitid=self.unitid, 
                        outcome=self.outcome, 
                        treatmentname=self.treat, 
                        treated_unit_name=prepared_data["treated_unit_name"], # type: ignore # Name of the treated unit
                        y=prepared_data["y"], # type: ignore # Observed outcomes of the treated unit
                        cf_list=[np.array(plot_counterfactual_series)], # List of counterfactual series (PDA usually has one)
                        counterfactual_names=[counterfactual_plot_name], # Names for the legend
                        method=plot_estimation_method_name, # Method name for title/saving
                        treatedcolor=self.treated_color,
                        counterfactualcolors=(
                            # Ensure counterfactualcolors is a list, even if a single color string is provided in config
